...
class HttpServer(controller: Try[IController]) extends Reactor {
 var icontroller: IController = _
  ...
  val route: Route = get {
    pathSingleSlash {
      complete(HttpEntity(ContentTypes.`text/plain(UTF-8)`, "<h1>HTWG Muehle</h1>"))
    }
    path("mills") {
      complete(HttpEntity(ContentTypes.`text/html(UTF-8)`, "<h1>HTWG Muehle</h1>" + icontroller.gridToHTML))
    } ~
      path("mills" / Segment) { command => {
        process_cmd(command)
        complete(HttpEntity(ContentTypes.`text/html(UTF-8)`, "<h1>HTWG Muehle</h1>" + command + icontroller.gridToHTML)
        )
      }
      }
  }
